...
if [ "$1" = "--popup" ]; then
  ...
  selected=$(build_list | fzf \
    ...
    --bind "alt-j:execute-silent($SCRIPTS_DIR/session-move.sh down {2})+reload(bash -c build_list)" \
    --bind "alt-k:execute-silent($SCRIPTS_DIR/session-move.sh up {2})+reload(bash -c build_list)" \
